...
class GatewayWarmupError(RuntimeError):
    """Gateway warmup 확인 중 예기치 못한 내부 오류."""


def check_gateway_warmup(ip, port, required_num_clnt, required_ops):
    """
    Gateway warmup 상태를 확인한다.

    Returns:
        bool:
            True  - Warmup 완료
            False - Warmup 미완료(통신 실패 포함)
    ...
    try:
        with GwCmd(ip, port) as gw_cmd:
            num_clients = gw_cmd.info_num_of_clients()
            ops = gw_cmd.info_ops()

        # 원본 코드의 계약(>)을 그대로 유지
        is_warm = (
            num_clients > required_num_clnt
            and ops > required_ops
        )

        logger.info(
            "[%s:%s] client=%d/%d ops=%d/%d warmup=%s",
            ip,
            port,
            num_clients,
            required_num_clnt,
        ...
        logger.warning(
            "Gateway communication failed (%s:%s): %s",
            ip,
            port,
            exc,
        )
        ...
        logger.exception(
            "Unexpected error while checking gateway warmup (%s:%s)",
            ip,
            port,
        )
