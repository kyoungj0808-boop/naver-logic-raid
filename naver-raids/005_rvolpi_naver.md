...
연구용 평가 엔진으로서는 충실하지만, 운영 관점에서는 방어적 설계가 부족해 입력 데이터 오류(KeyError), 멀티프로세싱 리소스 누수, 하드코딩된 환경 의존성이 전체 평가 신뢰도를 흔들 수 있는 구조다.
...
logger = logging.getLogger("PQ_EVAL_ENGINE")
...
def retry_on_runtime_error(max_retry=3):
...
@retry_on_runtime_error(MAX_RETRY)
def safe_pq_worker(
        proc_id,
        annotation_set,
        gt_folder,
    ...
    logger.info(
        f"[Worker-{proc_id}] START "
        f"images={len(annotation_set)}"
    )
    ...
    try:
        ...
        if result is None:
            ...
            raise RuntimeError(
                f"Worker-{proc_id} returned None"
            )


        logger.info(
            f"[Worker-{proc_id}] COMPLETE"
        )
...
def pq_compute_multi_core(
    ...
    if len(matched_annotations_list) == 0:
    ...
    cpu_num = min(
        ...
        MAX_WORKERS,
        len(matched_annotations_list)
    )


    logger.info(
        f"Starting PQ engine "
        f"workers={cpu_num}"
    )
    ...
    try:
        ...
        logger.info(
            "PQ ENGINE FINISHED SUCCESSFULLY"
        )
...
✅ Retry → Exponential Backoff 적용
✅ multiprocessing.Pool 종료/정리 보장으로 Zombie Process 방지
✅ Worker 실패 시 부분 PQ 결과 생성 방지
✅ result.get() Timeout 예외 명확화
✅ cpu_count() → 최대 Worker 제한 적용
✅ 빈 작업 chunk 제거
✅ Worker traceback 보존 (logger.exception)
✅ 입력 데이터 검증 추가
✅ Worker 반환값 None 검증 추가
