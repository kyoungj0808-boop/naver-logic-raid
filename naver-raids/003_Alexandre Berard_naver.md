...
분산 학습 프로세스 생명주기 관리가 단일 예외를 전체 엔진 장애로 확산시킬 수 있는 구조이며, 장기간·대규모 GPU 환경에서는 실패 전파, NCCL 종료, 체크포인트 복구 계층이 부족해 데드락과 데이터 무결성 손상 위험을 내포한다.
...
class DistributedOrchestrator:
    ...
    def __init__(
        ...
        self.heartbeat = self.manager.dict()
    ...
    def spawn_workers(
        ...
        for rank in reversed(range(node_size)):
            ...
            worker_cfg = self.cfg.clone()

            worker_cfg.device = (
                f"cuda:{start_device + rank}"
            )

            worker_cfg.rank = rank
            ...
            self.heartbeat[rank] = time.time()
            self.progress[rank] = 0
    ...
    def watchdog(self):
        ...
        while not self.shutdown_event.is_set():
            ...
            # 2. Distributed progress detection

            for rank, timestamp in self.heartbeat.items():
