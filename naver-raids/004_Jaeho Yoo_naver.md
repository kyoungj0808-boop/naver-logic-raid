...
고난도 HiveServer2/Kerberos 통신을 처리하는 강력한 엔진이지만, Atomic Swap 기반의 원자성 보장과 예외 격리 래퍼가 부재해 중간 셧다운 시 데이터 무결성과 런타임 상태 복구를 보장하지 못하는 '출력 성능은 뛰어나나 장애 내성이 부족한 엔진'이다.
...
def safe_thrift_operation(func):
    """
    Thrift 통신 계층 보호 Decorator
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            _logger.exception(
                "[THRIFT FAILURE] %s failed: %s",
                func.__name__,
                e
            )

            # 통신 객체가 깨졌음을 명시적으로 기록
            obj = args[0]

            if hasattr(obj, "_connection"):
                obj._connection._mark_unhealthy()
...
class Connection(object):
    ...
    def set_auth_setting(self):
        ...
        if self.service_mode != 'http' or self.auth != 'KERBEROS':
            return

        acquired = self.auth_lock.acquire(timeout=5)

        if not acquired:
            raise TimeoutError(
                "Kerberos authentication lock timeout"
            )

        old_client = self.kerb_client
        old_expire = self.expired_time
        ...
        try:
            ...
            if (
                self.kerb_client is None
                or self.expired_time <= now
            ):

                if self.keytab_file:
                    client, expire = (
                        self._get_krb_client_with_keytab(
                            self.username,
                            self.realm
                        )
                    )
                    ...
                    if not c_path:
                        c_path = (
                            _get_krb5_config_value_with_variable(
                                self.krb_conf
                                .lib_defaults
                                .default_ccache_name
                            )
                    ...
                    if not c_path:
                        raise IOError(
                            "Kerberos credential cache missing"
                        )

                    client, expire = (
                        self._get_krb_client_with_ccache(
                            self.username,
                            self.realm,
                            c_path
                        )
                ...
                # 인증 객체 교체는 마지막 순간에 수행
                self.kerb_client = client
                self.expired_time = expire
            ...
            spn.service = (
                self.kerberos_service_name
                if '/' in self.kerberos_service_name
                else
                self.kerberos_service_name
                + '/'
                + self.host
            ...
            tgs, enc, key = (
                self.kerb_client.get_TGS(spn)
            )

            token = (
                self.kerb_client.construct_apreq(
                    tgs=tgs,
                    encTGSRepPart=enc,
                    sessionkey=key,
                    flags=0
            ...
            # 인증 실패 시 이전 정상 상태 유지
            self.kerb_client = old_client
            self.expired_time = old_expire
...
class Cursor(common.DBAPICursor):
    ...
    def _reset_state(self):
        ...
        try:
            ...
            request = (
                ttypes.TCloseOperationReq(handle)
            )
            ...
            _logger.warning(
                ...
                "Handle may remain server-side: %s",
                e
            )
