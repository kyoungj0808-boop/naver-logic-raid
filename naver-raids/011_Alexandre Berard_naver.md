...
대규모 분산 학습 환경에서 데이터 병목을 줄이기 위해 설계된, 성능·메모리 최적화와 멀티프로세싱 파이프라인을 균형 있게 구현한 프로덕션 수준의 데이터셋 엔진입니다.
...
def safe_save_index(index_data: dict, path: str):
    """
    운영 환경에서의 데이터 무결성과 장애 복구 가능성을 높인 저장 로직.
    - 파일 객체 직접 전달을 통해 flush/fsync의 정합성을 보장합니다.
    """
    # 1. 파일 시스템 수준의 원자적 저장 보장을 위한 임시 파일 생성
    tmp_path = None
    try:
        with tempfile.NamedTemporaryFile(
            ...
            # 2. 파일 객체를 직접 전달하여 쓰기 핸들 제어권 확보
            torch.save(index_data, tmp)
            
            # 3. 데이터 동기화: 캐시가 아닌 물리적 디스크 쓰기 보장
            tmp.flush()
            os.fsync(tmp.fileno())
        
        # 4. 원자적 교체 (운영체제 수준의 Atomic Rename)
        os.replace(tmp_path, path)
        logger.debug(f"Index successfully saved to {path}")
        
    except (OSError, IOError) as e:
        logger.exception(f"Failed to save index to {path}")
        # 운영상 치명적인 오류만 상위로 전파
        if getattr(e, "errno", None) in (errno.ENOSPC, errno.EACCES):
            raise
            
    finally:
        # 5. 작업 실패 여부와 관계없이 임시 파일 정리
...
최종 개선사항
✅ 원자적 파일 교체(os.replace) 적용으로 저장 중 비정상 종료 시 기존 인덱스 파일 손상 가능성 감소
✅ 임시 파일(tempfile.NamedTemporaryFile) 사용으로 다중 프로세스 환경에서 파일명 충돌 위험 완화
✅ 파일 객체 기반 torch.save()와 flush()·fsync() 적용으로 디스크 기록 신뢰성 강화
✅ logger.exception() 도입으로 예외 발생 시 스택 트레이스를 포함한 원인 추적성 향상
✅ finally 블록을 통한 임시 파일 정리로 실패 시 잔여 파일 누적 방지
✅ getattr(e, "errno", None) 사용으로 플랫폼별 예외 객체 차이를 고려한 안전한 오류 판별
✅ 기존 저장 API와 동작 방식을 유지하면서 운영 안정성과 데이터 무결성만 보강

기존 저장 로직의 동작은 유지하면서 원자적 저장, 데이터 무결성, 장애 복구 및 디버깅 편의성을 보강해 운영 환경에 더욱 적합한 저장 로직으로 개선한 유지보수 중심의 패치입니다.
