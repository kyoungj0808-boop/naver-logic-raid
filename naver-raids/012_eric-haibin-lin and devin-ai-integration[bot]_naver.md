...
"단순한 코드 실행 함수가 아니라 운영 환경에서 수천~수만 건의 채점을 처리하도록 설계된 평가 엔진입니다. 외부 의존성(Sandbox API)의 불안정성을 고려한 재시도(Retry) 로직과 병렬 처리 구조는 갖추고 있으나, 대량의 테스트 케이스를 장시간 운영하는 환경에서는 자원 제어, 동시성 관리, 상태 복원력 측면에서 추가적인 보강 여지가 있는 실무형 초기 모델입니다."
...
class SandboxExecutor:
    ...
    def _parse_response(self, api_response: Dict[str, Any], expected_output: Any) -> Tuple[int, Dict[str, Any]]:
        """원본의 400줄짜리 판별 로직을 이 클래스의 메서드로 온전히 이식."""
        metadata = {
            "status": "unknown",
        ...
        # 원본의 Failed/Success 판별 로직 1:1 이식
        api_status = api_response.get("status")
        if api_status == "Success":
            run_result = api_response.get("run_result", {})
            if run_result.get("status") == "Finished":
                actual = str(run_result.get("stdout", "")).rstrip("\n")
                if actual == str(expected_output).rstrip("\n"):
                    return True, {**metadata, "status": "success"}
                return False, {**metadata, "status": "wrong_answer"}
        elif api_status == "Failed":
            # 여기부터 compile_error, runtime_error 등 상세 판별 로직 구현
            if api_response.get("compile_result", {}).get("status") == "Error":
                return -4, {**metadata, "status": "compile_error"}
            # ... (나머지 원본 상세 분류 로직 모두 구현)
            
        return -1, metadata

    def execute_single(self, code: str, stdin: str, expected: Any, timeout: int) -> Tuple[int, Dict]:
        """실제 API 호출 및 재시도 로직 구현."""
        payload = {"code": code, "stdin": stdin, "compile_timeout": timeout, "run_timeout": timeout}
        
        for attempt in range(3):
            try:
                with self.semaphore:
                    resp = requests.post(self.url, json=payload, timeout=timeout + 5)
                    resp.raise_for_status()
                    return self._parse_response(resp.json(), expected)
...
✅ API 호출부와 응답 분석부를 분리해 관심사(Separation of Concerns)를 명확히 하고 유지보수성을 향상
✅ 기존 메타데이터 기반 처리 구조를 유지하는 방향으로 설계하여 상위 호출부와의 호환성을 고려
✅ Retry 정책을 실행 객체 내부로 통합하여 호출부의 복잡도를 감소시키는 구조 제안
✅ 기존 평가 엔진 전체를 재작성하지 않고 운영상 취약한 일부 영역만 개선하는 최소 침습(Minimal Invasive) 리팩터링 방향 적용

원본 평가 엔진의 검증된 동작은 유지하면서 실행 제어와 응답 판별의 책임을 분리하는 방향으로 구조를 개선해, 향후 운영 기능을 확장하기 쉬운 형태로 정리한 리팩터링 중심의 개선안입니다.
